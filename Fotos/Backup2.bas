...
Sub Process_Globals
	...
	Type TipoOperacion1 (FotosCamara As Int, VideosCamara As Int, ImagenesWhatsApp As Int, VideosWhatsApp As Int)
	
	Dim Operacion As TipoOperacion1
...
'	Private BtnListarPorFecha As Button
'	Private BtnListarDsdServer As Button
...
Sub Activity_Create(FirstTime As Boolean)
	...
	IniciarProceso(DatosGlobales.ColMobilRutaOrigen)
...
Sub GuardarArchivoEnElServidor (PathDelServidor As String, Ip As String, Puerto As Int, Usuario As String, Contra As String, RutaMobile As String, ArchivoMobileFull As String, TipoDeOperacion As Int) As ResumableSub
	...
	If File.Exists(PathDelServidor & "/" & Path & "/" & FechaArchivo,"") = False Then
		...
		'FTP3.SendCommand("MKD", PathDelServidor & "/" & Path) ' --> asi es como funcionaba 26/08/2023
...
'		If Success Then
'			Log("Creo carpeta: " & PathDelServidor & "/" & Path)
'		End If
	Else
...
'		FTP3.SendCommand("MKD", PathDelServidor & "/" & Path & "/" & FechaArchivo) '--> asi es como funcionaba 26/08/2023
'		If Success Then
'			Log("Creo carpeta: " & PathDelServidor & "/" & Path & "/" & FechaArchivo)
'		End If
	Else
...
'	
	...
	Dim rs As ResumableSub = EncontroArchivoEnServidor(PathDelServidor & "/" & Path & "/" & FechaArchivo & "/", ArchivoMobile) 'ej: FTP_1("./H/DiscoCompartido/RiBackup/Fotos/20230924/", "Foto_Bath_Puerta.jpg")
...
Sub IniciarProceso (ColMobilRutaOrigen As List)
	...
	ListaFotos.Clear
	ListaVideos.Clear
	'ListaCompleta = File.ListFiles(PathDelCelular)
	...
	'Msgbox("Cantidad total: " & CantidadTotal, "Mensaje del sitema")
	...
	For i = 0 To ColMobilRutaOrigen.Size -1
		...
		Try
			...
			'Log("Paso bien la lista: " & Tipo)
		
			
			For j = 0 To ListaGenerica.Size -1
				'LblTexto.Text = "Procesando lista: " & Tipo & " - Ubicación: " & Ruta & CRLF & i & "/" & CantidadTotal
...
Public Sub EncontroArchivoEnServidor(unaRutaServer As String, unArchivo As String) As ResumableSub
